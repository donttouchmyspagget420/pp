...
@section('content')
  ...
  <form action="/publicacion/edit" method="post" enctype="multipart/form-data">
    @csrf
    <input type="hidden" value="{{$pub['id']}}">
    <div class="container row mx-auto mt-5">
      ...
      <div class="col-12 col-md-6 d-flex justify-content-around align-items-center">
        <div class="row">
        <input class="form-control form-control-lg mb-3" type="text" placeholder=".form-control-lg" value="{{ $pub->titulo }}" name="titulo" required aria-label=".form-control-lg example">
        <select class="form-select" aria-label="Default select example" id="sel"   required name="categoria" >
            <option selected value="{{ $pub['categorias']['nombre'] }}">{{ $pub['categorias']['nombre'] }}</option>
            @foreach($cats as $cat)
